...
for (chr in unique(results$Chr)){
	...
	if (genomeBuild == "hg38" && file.exists(cytobandFile)){
		sl <- seqlengths(seqinfo[chr])
  		#pI <- plotIdeogram.hg38(chr, cytoband=cytoband, seqinfo=seqinfo, xlim=c(0, max(sl)), unit="bp", label.y=label.y, new=FALSE, ylim=ylim)	
  	}else{
		#pI <- plotIdeogram(chr, build="hg19", unit="bp", label.y=label.y, new=FALSE, ylim=ylim)
  	}
